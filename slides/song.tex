...
\begin{frame}{Scott tripos}
	...
	\vfill
	\begin{enumerate}
		\item A predicate on $X : \Set$ is a map $\varphi : X \to \alg F$ (\textit\gold{$W$-predicates}).
		\item $\land, \lor, \hey$ are pointwise essential intersection/union/implication.
		\item $\true = [W] = $ \gold{almost everywhere},\\
		$\false = [\varnothing] =$ \gold{almost nowhere}.
		\item Quantifiers:
		\begin{equation*}
...
% 		\gold{$\qquad \rightsquigarrow$ `$x \sim_X x$' is thought of as `$x \in X$'}.
...
\begin{frame}{Geometry}
	...
	\begin{center}
		\begin{tabularx}{\columnwidth}{C|c}
			...
			& (\gold{kind of})\\[2ex]
			Grothendieck topos $\Sh S$ & \gold{Kolmogorov topos} $\Sh_\aetop W$
		\end{tabularx}
	\end{center}
	\vfill
\end{frame}
...
\begin{frame}{Measurability structures}
	...
	\end{eqalign*}
	%\gold{We can interpret it as a `\textit{possibly at $t$}' modal operator.}
	...
	\begin{eqalign*}
		...
		\gold{\lozenge_f A \nto{g} E} &\longmapsto \gold{A \into \lozenge_f A \nto{g} E}
	\end{eqalign*}

	\vfill
	Notice: $\lozenge_f A \subseteq V$, while $A \subseteq W$.\\
	Hence \gold{$\hat f$ picks $V$-measurable elements among $W$-measurable elements.}
\end{frame}
...
\begin{frame}{Internal notions}
	...
	\vfill
	If there's time, as an application, we'll define \gold{\bfseries martingales}.
\end{frame}
...
\begin{frame}{Random variables}
	...
	\begin{equation*}
		\RNO_W = \O_W(\R)
	\end{equation*}
	Hence this confirms \gold{`random variables' are indeed `variables' in a `random' setting.}
	...
	% \vfill
	% Also, $L^p_\gold{\mathsf{loc}}$ spaces become subsets of the real numbers.
\end{frame}
...
\begin{frame}{Integration}
	...
	\begin{eqalign*}
		...
		(X, \mu) &\longmapsto \underbrace{\bigvee_{q \leq X} q \mu}_\gold{\int_{-} X\,\de\mu}
	\end{eqalign*}
\end{frame}
...
\begin{frame}{Radon--Nikodym}
	...
	\vfill
	It says every \gold{absolutely continuous measure wrt $\mu$ has a $\mu$-density}.
\end{frame}
...
\begin{frame}{Filtrations}
	To define \textbf{adapted} processes, we need to express `\textit{measurability at $t$}' for elements in $\O_W(E)$.
...
\begin{frame}{Filtrations}
	...
	\begin{equation*}
		\hat 1_t^E : \underbrace{\O_t^*(E)}_{:= 1_t^*\O_t(E)} \longto \O_\infty(E)
	\end{equation*}
	\gold{$\rightsquigarrow$ $\O^*_t(E)$ contains $\F_t$-measurable elements}.
\end{frame}
...
% 		\gold{\lozenge_t A \nto{f} E} &\longmapsto \gold{A \into \lozenge_t A \nto{f} E}
% 	\end{eqalign*}
% 	Notice: $\lozenge_t A \subseteq W_t$, while $A \subseteq W_\infty$\\
% 	\hspace*{3ex} \gold{$\rightsquigarrow$ $\O^*_t(E)$ contains $\F_t$-measurable elements}.
% \end{frame}

\begin{frame}{Filtrations}
	For each $t \in I$, let \gold{$\msbl(t)$} be the image of $\hat 1_t$.\\[1ex]
	...
	\vfill
	\gold{This recovers $\F_\tau$-measurability for a \textbf{random time} $\tau : W \to I$}.
\end{frame}
...
% 		\item An internal definition of `adaptedness' $\rightsquigarrow$ \gold{filtrations}
...
\begin{frame}{Conditional expectation}
	...
	\end{eqalign*}
	\begin{center}
		\gold{best approximation at $t$ of $X$'s average}.
	\end{center}
\end{frame}
...
\begin{frame}{Martingales}
	\begin{definition}
		...
		\begin{equation*}
			W \forces \forall t \tin \Delta I\,\, \forall s \geq t,\ \Exp[X(s) | \F_t]_\infty = X(t),
		\end{equation*}
	\end{definition}
\end{frame}
